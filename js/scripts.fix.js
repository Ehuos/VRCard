...
$(document).ready(function(){
				...
				if ($("#gmap").is(":empty")){
					new Maplace({
						map_options: {
							...
							set_center: [30.3084933, 120.3428415],
							zoom: 4
						}
					}).Load(); 
				}
	...
	jQuery(window).on("hashchange load", function(e) {
        ...
        if (page === 'blog') {
		    in_post_view = false;
		    $(".blog_posts .post").find(".hidden_body").hide();
		    $(".blog_posts .post").find(".preview_body").show();
		    ...
		    $(".blog_posts .post").removeClass("flipInX");
		    $(".blog_posts .post").show();
		    $(".blog_posts").find(".load_more_posts").show();
		    $(".blog_posts").find(".back_to_blog").hide();
        }

        var blog_reg = /^blog_(\d+)/;
        var blog_match = blog_reg.exec(page);
        if (blog_match) {
            page = 'blog';
            var blog_id = parseInt(blog_match[1], 10);
            var blog_post_data = $(".blog_posts .post_data").eq(blog_id);
		    in_post_view = true;
		    $(".blog_posts .post").hide();
		    $(blog_post_data).find(".hidden_body").show();
		    $(blog_post_data).find(".preview_body").hide();
		    $(blog_post_data).find(".btns .read_more_btn").hide();
		    $(blog_post_data).parents(".post").removeClass("animate flipInX").addClass("fullOpacity");
		    $(blog_post_data).parents(".post").fadeIn("fast");
		    $(blog_post_data).prev().show();
		    $(blog_post_data).parents(".blog_posts").find(".load_more_posts").hide();
		    $(blog_post_data).parents(".blog_posts").find(".back_to_blog").show();
        }
